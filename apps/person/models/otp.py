...
class AbstractOTPFactory(models.Model):
    ...
    def validate(self):
        if self.is_used:
            raise ValidationError(_(u"Has used on %s" % (self.date_updated)))

        if self.is_verified:
            raise ValidationError(_(u"Has verified on %s" % (self.date_updated)))

        if self.is_expired:
            raise ValidationError(_(u"Has expired on %s" % (self.date_updated)))
