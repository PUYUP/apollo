...
from apps.person.utils.auth import get_users_by_email, clear_otp_session

OTPFactory = get_model('person', 'OTPFactory')


def generate_token_uid_by_email(email, request):
    users = get_users_by_email(email)
    for user in users:
        token = default_token_generator.make_token(user)
        uidb64 = urlsafe_base64_encode(force_bytes(user.pk))

        request.session['password_recovery_token'] = token
        request.session['password_recovery_uidb64'] = uidb64
...
class OTPFactoryApiView(viewsets.ViewSet):
    ...
    def generate_otp_session(self, request, serializer):
        # save to session for a time
        challenge = serializer.data.get('challenge', None)
        email = serializer.data.get('email', None)

        request.session.set_expiry(settings.OTP_COOKIES_EXPIRED) # 5 minutes
        for field in OTP_SESSION_FIELDS:
            v = serializer.data.get(field, None)
            if not isinstance(v, str):
                v = str(v)
            request.session['otp_request_%s' % field] = v

        # password recovery has token
        if email and challenge == PASSWORD_RECOVERY:
            generate_token_uid_by_email(email, request)
    ...
    def create(self, request, format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            # store otp data to session
            self.generate_otp_session(request, serializer)
    ...
    def validate(self, request):
        ...
        # prepare session after validate
        session_key = 'otp_validate'

        # maybe used for other purpose ex: register, password reset, other
        request.session.set_expiry(settings.OTP_COOKIES_EXPIRED) # 5 minutes
        for field in OTP_SESSION_FIELDS:
            v = getattr(otp_obj, field, None)
            k = '%s_%s' % (session_key, field)
            if not isinstance(v, str):
                v = str(v)
            request.session[k] = v

        # clear otp `request` session
